...
with overview_tab:
    ...
    st.markdown(
        ...
        "支持中文输入法：拼音组词和选字时不会提前搜索，文字确认上屏后才显示结果。"
        "搜索框使用 Streamlit 原生文本控件，不放在 iframe 中；拼音组词和选字期间不会触发页面重绘，中文确认上屏后才查询。若浏览器没有发送组合事件，按 Enter 仍会直接检索并跳到第一个匹配项。候选列表向上悬浮，不会改变底栏高度或把按钮挤到下一行。"
