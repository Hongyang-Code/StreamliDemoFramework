...
搜索输入使用与“标签名称”相同的 Streamlit 原生文本控件，不再放进任何 iframe 或自定义输入组件。中文拼音组词和选字期间不会向后端发送检索请求，文字确认上屏后才查询；按 Enter 还提供完全不依赖前端组合事件的可靠检索与跳转后备路径。输入框、行列、翻页、标记开关和刷新按钮在视觉上保持于同一条底栏中，候选列表向上悬浮，不会挤动底栏布局。
