...
- 所有样本在一个连续页面中展示，不使用分页；列数不设上限，实际行数自动计算。
- 工具栏持续固定在浏览器底部；刷新文件列表后在同一连续页面更新所有内容。
- 大图片自动缩放压缩、大视频自动转码、超大文本按上限截断。
- 原始媒体不进入无界内存缓存，全部预览仍受单样本和单页总载荷预算约束。
...
| `--page-payload-limit-mb N` | 否 | 连续单页的总预览预算，默认 128 MB |
...
1. 在左侧创建标签；点击彩色标签名称即可选中，点击“编辑”设置该标签自己的颜色、角标/外框形式或名称，点击“删除”直接删除。
2. 左键点击样本，立即添加或取消当前标签。
3. 图片卡片悬停后点击右上角的查看按钮，或右键选择“单独查看”，可缩放、滚轮缩放和拖拽。
4. 右键点击样本，选择“单个样本备注”。
5. 上下滚动查看全部样本；页面不再划分页码。
6. 修改底部列数后，实际行数和连续网格会立即重算；列数不设上限。
7. 每个标签可在“编辑”中独立选择“角标”或“外框”；外框从内到外、角标从左到右均采用固定标签顺序，不受点击先后影响。
8. 取消“显示标记”可临时隐藏角标或外框，不会删除数据。
9. 点击底栏“刷新”可扫描新增或移除的文件，并在同一页面更新全部内容。

左侧“打开操作指南”会进入带有界面示例和故障排查说明的独立帮助页面。
...
每个标签独立的角标/外框属性保存在 `label/label_settings.json`，重命名或删除标签时会同步更新：
...
- 单页总预算会在全部样本之间分配，防止一次展示大量内容时传输量失控。
...
其中图片目录包含 30 个平铺样本，便于检查连续单页布局和大量卡片时的交互。第一次运行任一模板时，对应目录下会自动创建 `label/`。这些运行时标签目录已被 Git 忽略。
