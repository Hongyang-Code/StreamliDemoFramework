...
- 多标签可选择右下角彩色圆形角标或多层彩色外框，并可一键隐藏；两种样式始终按照侧栏标签顺序排列。
- 左键即时标记；连续点击先在浏览器中无等待反馈，再合并为一次幂等、原子化批量写入。
- 右键样本打开“单个样本备注”；当前备注和完整历史集中保存到一个 JSON。
- 图片可单独打开仔细查看，支持按钮或滚轮缩放以及拖拽平移。
- A/D 键翻页，行列数不设上限，变化后立即更新当前页和总页数。
...
1. 在左侧创建标签；每个标签都是可展开的独立卡片，展开后可以选择、修改颜色、重命名或删除，再次点击卡片即可收起。
2. 左键点击样本，立即添加或取消当前标签。
3. 图片卡片悬停后点击右上角的查看按钮，或右键选择“单独查看”，可缩放、滚轮缩放和拖拽。
4. 右键点击样本，选择“单个样本备注”。
5. 使用 A/D 或底部按钮翻页。
6. 修改底部行数、列数后，网格和总页数会立即重算；行列数不设上限。
7. 在底栏选择“角标”或“外框”；外框从内到外、角标从左到右均采用固定标签顺序，不受点击先后影响。
