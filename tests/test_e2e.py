...
def test_browser_label_note_layout_badges_and_keyboard(tmp_path: Path):
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            expect(cards).to_have_count(30, timeout=30_000)
            ...
            assert toolbar.bounding_box()["y"] >= 900
            main_panel = page.locator('[data-testid="stMain"]')
            main_panel.evaluate("element => { element.scrollTop = 900; }")
            page.wait_for_timeout(100)
            assert toolbar.bounding_box()["y"] >= 900
            main_panel.evaluate("element => { element.scrollTop = 0; }")
            ...
            cols.fill("4")
            cols.press("Tab")
            expect(page.locator(".toolbar label", has_text="列").locator("input")).to_have_value("4", timeout=10_000)
            expect(page.locator(".sample-card")).to_have_count(30)
            expect(page.get_by_text("单页全部显示", exact=False)).to_be_visible()
            expect(page.get_by_text("上一页", exact=False)).to_have_count(0)
            expect(page.get_by_text("下一页", exact=False)).to_have_count(0)
            expect(page.get_by_text("页码", exact=False)).to_have_count(0)
